# -*- coding: utf-8 -*-
"""final_xai_rainy_to_clear.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14uQlkUW6rEy8GiCNp5HkpnYfUNj5aV4b
"""
...
ZIP_PATH = f'{DRIVE_ROOT}/FDL_DS/rainy_models_and_ds/rainy-to-clear.zip'   # path to your zip file
EXTRACT_DIR = '/content/dataset'            # where zip will be extracted
WEIGHTS_CYCLEGAN = f'{DRIVE_ROOT}/FDL_DS/rainy_models_and_ds/G_BA_epoch_200_cyclegan_rainy_to_clear.pth'   # cycle generator weights
WEIGHTS_FASTCUT = f'{DRIVE_ROOT}/FDL_DS/rainy_models_and_ds/G_epoch_195_fastcut_rainy_to_clear.pth'     # fastcut generator weights
WEIGHTS_RESNET = f'{DRIVE_ROOT}/FDL_DS/rainy_models_and_ds/G_epoch_192_resnet_rainy_to_clear.pth'       # resnet generator weights
OUTPUT_DIR = f'{DRIVE_ROOT}/xai_outputs/rainy_to_clear'                      # where outputs will be saved
...
rainy_images = [p for p in all_images if "rain" in os.path.basename(os.path.dirname(p)).lower()]

assert len(rainy_images) > 0, f"No rainy images found under {DATASET_DIR}. Check your folder names."

# summary print
print(f"Found {len(rainy_images)} snowy images for generation.")
print(f"Example snowy folder: {os.path.dirname(rainy_images[0])}")

# shuffle and select first N_GENERATE
random.seed(42)
random.shuffle(rainy_images)
generate_list = rainy_images[:N_GENERATE]
print(f"Will generate {len(generate_list)} images (only rainy domain used).")

# ---- Cell: GeneratorCycleGAN ----
import torch
...
class GeneratorCycleGAN(nn.Module):
    """
    CycleGAN-style ResNet generator (default 6 residual blocks).
    Supports return_features for compatibility with xAI pipelines.
    """
    def __init__(self, in_ch=3, out_ch=3, n_res_blocks=6, ngf=64):
        super().__init__()
        enc = [
        ...
        # smaller number of res blocks (classical CycleGAN uses 6 or 9; we default to 6)
        self.middle = nn.Sequential(*[ResBlockSmall(ngf*4) for _ in range(n_res_blocks)])

        dec = [
            nn.ConvTranspose2d(ngf*4, ngf*2, 3, stride=2, padding=1, output_padding=1), nn.InstanceNorm2d(ngf*2), nn.ReLU(True),
            nn.ConvTranspose2d(ngf*2, ngf, 3, stride=2, padding=1, output_padding=1),   nn.InstanceNorm2d(ngf),   nn.ReLU(True),
            nn.ReflectionPad2d(3),
...
# Example: instantiate
# G_cycle = GeneratorCycleGAN(n_res_blocks=6).to(device)

# ---- Cell: GeneratorFastCUT ----
import torch
import torch.nn as nn
...
class GeneratorFastCUT(nn.Module):
    """
    FastCUT-style ResNet generator.
    API: G = GeneratorFastCUT(n_res_blocks=9)
         out = G(x) or out, feats = G(x, return_features=True)
    """
    def __init__(self, in_ch=3, out_ch=3, n_res_blocks=9, ngf=64):
        ...
        self.middle = nn.Sequential(*[ResBlock(ngf*4) for _ in range(n_res_blocks)])
...
# Example: instantiate
# G_fastcut = GeneratorFastCUT(n_res_blocks=9).to(device)

# ---- Cell: GeneratorResNet ----
import torch
import torch.nn as nn

class ResBlockFull(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.block = nn.Sequential(
            nn.ReflectionPad2d(1),
...
class GeneratorResNet(nn.Module):
    """
    ResNet generator used in the 'unpaired' model training with perceptual loss.
    Default to n_res_blocks=9 to match training setups that used 9 res-blocks.
    """
    def __init__(self, in_ch=3, out_ch=3, n_res_blocks=9, ngf=64):
        super().__init__()
        enc = [
        ...
        self.middle = nn.Sequential(*[ResBlockFull(ngf*4) for _ in range(n_res_blocks)])
...
# Example: instantiate
# G_resnet = GeneratorResNet(n_res_blocks=9).to(device)
...
xai_index = 15   # change if you want a different image from the generated set
